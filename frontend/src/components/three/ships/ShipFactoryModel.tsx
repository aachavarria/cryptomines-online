...
interface ShipFactoryModelProps {
...
/**
 * Ship Factory: industrial building on the planet surface.
 * Tall hangar-like structure with assembly gantry, rotating crane arm,
 * and glowing production bays. Visually distinct from generic box buildings.
 */
export default function ShipFactoryModel({
  position = [0, 0, 0],
  scale = 1,
  emissiveIntensity = 0.15,
}: ShipFactoryModelProps) {
  const groupRef = useRef<Group>(null)
  const craneRef = useRef<Group>(null)
  const bayLightRef = useRef<Mesh>(null)

  const color = '#ff4444' // military category
  ...
  useFrame((state) => {
    ...
    if (craneRef.current) {
      craneRef.current.rotation.y = t * 0.3
    }
  ...
  return (
    <group ref={groupRef} position={position} scale={scale}>
      {/* Main hangar body - wide, industrial */}
