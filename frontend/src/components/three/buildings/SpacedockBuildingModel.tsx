...
export default function SpacedockBuildingModel({ position = [0, 0, 0], scale = 1, level = 1, animate = true }: BuildingComponentProps) {
  ...
  return (
    <group ref={groupRef} position={position} scale={scale}>
      {/* Central pillar / support column (tapered) */}
        ...
        {[0, Math.PI / 2, Math.PI, (3 * Math.PI) / 2].map((angle, i) => (
          ...
          >
            <mesh castShadow>
              ...
              <meshStandardMaterial color="#3a4a5a" metalness={0.9} roughness={0.15} />
            </mesh>
      ...
      {[0, (2 * Math.PI) / 3, (4 * Math.PI) / 3].map((angle, i) => (
        <mesh
          key={`strut-${i}`}
          position={[Math.cos(angle) * 0.6, 2.3, Math.sin(angle) * 0.6]}
      ...
      {/* Base energy conduit lines */}
      {[0, Math.PI / 2, Math.PI, (3 * Math.PI) / 2].map((angle, i) => (
        <mesh
          key={`conduit-${i}`}
          position={[Math.cos(angle) * 0.8, 0.05, Math.sin(angle) * 0.8]}
