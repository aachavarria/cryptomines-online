...
function CivicCenterGLB({
  ...
  const { scene } = useGLTF("/assets/gbl/buildings/civic-center.glb");
  ...
  return (
    <group ref={groupRef} position={position} scale={scale}>
      <primitive object={clonedScene} scale={16} position={[0, 0, 0]} />
    </group>
  );
}

export default function CivicCenterModel(props: BuildingComponentProps) {
  return (
    <Suspense fallback={null}>
      <CivicCenterGLB {...props} />
    </Suspense>
  );
}

useGLTF.preload("/assets/gbl/buildings/civic-center.glb");
