...
function ResourceWarehouseGLB({
  ...
  const { scene } = useGLTF("/assets/gbl/buildings/warehouse.glb");
  ...
  return (
    <group ref={groupRef} position={position} scale={scale}>
      <primitive object={clonedScene} scale={12} position={[0, 0, 0]} />
    </group>
  );
}

export default function ResourceWarehouseModel(props: BuildingComponentProps) {
  return (
    <Suspense fallback={null}>
      <ResourceWarehouseGLB {...props} />
    </Suspense>
  );
}

useGLTF.preload("/assets/gbl/buildings/warehouse.glb");
